$._hexaV31 = {
  ...
  assembleAndApply:function(mapPath){
    ...
    try{
      ...
      stage='PLACE_VIDEO';vrows.sort(function(x,y){return Number(x.start_frame)-Number(y.start_frame);});var vp=0;for(i=0;i<vrows.length;i++){this._placeVideoItem(seq,vrows[i],itemMap);vp++;}report.stage=stage;report.video_items_placed=vp;if(reportPath)this._writeJSON(reportPath,report);
      stage='PLACE_AUDIO';var ap=0;for(i=0;i<(a.audio_items||[]).length;i++){this._placeAudioItem(seq,a.audio_items[i],itemMap);ap++;}report.stage=stage;report.audio_items_placed=ap;if(reportPath)this._writeJSON(reportPath,report);
