...
def build_preset_story_motion_plan(plan:dict, alignment:dict, vision_results:list[dict], rules_path, reference_path, *, fps:float=30.0, logger=None, calibration:dict|None=None):
    ...
    for scene in scenes:
        ...
        for u in units:
            ...
            beat=next((b for b in scene_beats if str(b.get('subject') or '').casefold() in str(u.get('semantic_unit_id') or '').casefold()),scene_beats[0] if scene_beats else {})
            e.update({'semantic_beat_id':beat.get('beat_id'),'semantic_beat_action':beat.get('action'),'semantic_beat_source':beat.get('source'),'semantic_beat_start_seconds':beat.get('start_seconds'),'semantic_beat_hit_seconds':beat.get('perceptual_hit_seconds'),'semantic_beat_end_seconds':beat.get('end_seconds')})
