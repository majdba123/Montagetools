...
def render_scene_media(render_edit_map,motion_plan,vision_results,text_plan,graphics_plan,out_dir,cache_dir,width=1920,height=1080,fps=30.0,logger=None):
    guarded_graphics=graphics_plan
    graphics_guard=None
    if motion_plan.get('interaction_engine') is not None:
        ...
        if logger and graphics_guard:
            logger.log('PASS','INTERACTION_GRAPHICS_GUARD',relationship_graphics=graphics_guard.get('relationship_graphic_count'),suppressed=graphics_guard.get('suppressed_count'),clamped=graphics_guard.get('clamped_count'))
    manifest=_base_render_scene_media(render_edit_map,motion_plan,vision_results,text_plan,guarded_graphics,out_dir,cache_dir,width=width,height=height,fps=fps,logger=logger)
    if motion_plan.get('interaction_engine') is not None:
