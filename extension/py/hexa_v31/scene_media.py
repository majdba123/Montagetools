...
from .editorial_runtime import promote_text_plan
...
def _renderer_dependency_signature()->str:
    """Version scene-media cache by every dependency that can change encoded pixels."""
    ...
    from hexa_v31.editorial_runtime import apply_editorial_runtime_state, promote_text_plan as _promote_text_plan
    ...
    for fn in (render_text_rgba,composition_state_at,_event_state,apply_editorial_runtime_state,_promote_text_plan):
...
def render_scene_media(render_edit_map,motion_plan,vision_results,text_plan,graphics_plan,out_dir,cache_dir,width=1920,height=1080,fps=30.0,logger=None):
    ...
    promoted_text_plan=promote_text_plan(text_plan,motion_plan)
    ...
    manifest=_base_render_scene_media(framed_render_map,motion_plan,vision_results,promoted_text_plan,guarded_graphics,out_dir,pixel_cache,width=width,height=height,fps=fps,logger=logger)
    manifest['renderer_dependency_sha256']=renderer_signature
    manifest['editorial_runtime_authority']='SEMANTIC_PHASE_PIXEL_DELIVERY_V1'
    manifest['editorial_typography_runtime_authority']='SEMANTIC_ROLE_MOTION_GRAPHICS_V1'
    ...
    write_json(composition_sources,dict(framed_render_map,
        composition_text_events=list((promoted_text_plan or {}).get('events') or []),
