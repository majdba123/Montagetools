...
def _solve_with_unknown_retry(model,cp_model):
    """Run the unchanged model with one bounded retry on UNKNOWN only.

    The serialized interaction plan must remain deterministic, so runtime timing
    and search telemetry are intentionally kept out of returned schedules.
    """
    attempts=[]
    ...
    for index,budget in enumerate(budgets):
        ...
        attempts.append({'attempt':index+1,'budget_seconds':float(budget),'status':_status_name(status,cp_model)})
...
def solve_interaction_schedule(intent:dict,candidate:dict,event_by_id:dict[str,dict],fps:float)->dict:
    ...
    solver,status,_attempts=_solve_with_unknown_retry(model,cp_model)
    common={'solver':'ORTOOLS_CP_SAT','steps':[],'deterministic':True}
    if status==cp_model.UNKNOWN:
