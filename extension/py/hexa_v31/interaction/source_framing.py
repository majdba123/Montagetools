...
VERSION='HEXA_VISIBLE_INK_SOURCE_FRAMING_V1'

def _ink_bbox(arr:np.ndarray):
    rgb=arr[:,:,:3].astype(np.int16);alpha=arr[:,:,3]
    # Preserve pale artwork: a pixel is ink if it is non-transparent and differs
    # measurably from canonical white in luminance or chroma.
...
def _safe_crop(arr:np.ndarray,bbox):
    ...
    # Do not reinterpret edge-touching artwork.  This is framing only, never
    # actor extraction or background removal.
...
def _planned_fit_percent(event:dict,crop_w:int,crop_h:int)->float|None:
    ...
    factor=min(desired_w/max(1.0,crop_w),desired_h/max(1.0,crop_h))
    # Keep bounded.  Extreme amplification is a sign that the planner/source
    # geometry disagree and should be handled upstream rather than hidden here.
...
def normalize_render_sources(render_edit_map:dict,cache_dir,logger=None)->tuple[dict,dict]:
    ...
    for event in out.get('events') or []:
        ...
        bbox=_ink_bbox(arr)
        safe=_safe_crop(arr,bbox) if bbox else None
        if safe is None:
        ...
        event['source_path']=str(dst);event['source_layer_path']=str(dst) if event.get('source_layer_path') else event.get('source_layer_path')
        event['base_fit_scale_percent']=round(fit,6)
        event['visible_ink_source_framing']={'version':VERSION,'original_source_path':str(p),'crop_box_px':[x0,y0,x1,y1],'original_size_px':[arr.shape[1],arr.shape[0]],'cropped_size_px':[x1-x0,y1-y0],'old_base_fit_scale_percent':old_scale,'new_base_fit_scale_percent':round(fit,6),'policy':'OUTER_WHITE_PADDING_ONLY__NO_BACKGROUND_REMOVAL'}
