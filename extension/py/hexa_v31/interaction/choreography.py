...
def _aligned_entry_manifestation(event:dict,intent:dict,phase:str,fps:float):
    ...
    return {'event_id':str(event.get('event_id')),'preset':name,'start_seconds':round(st,6),'end_seconds':round(en,6),'duration_seconds':round(dd,6),'phase':phase,
            ...
            'perceptual_impact_seconds':round(impact,6),'event_hit_seconds':round(event_hit,6),'key':'|'.join((str(event.get('event_id')),name,f'{st:.6f}',phase))}
...
def _matching_existing_relationship_action(event:dict,intent:dict,target:dict|None):
    ...
    return {'event_id':str(event.get('event_id')),'preset':name,'start_seconds':round(st,6),'end_seconds':round(en,6),'duration_seconds':round(en-st,6),'phase':'ACTION',
            ...
            'original_relationship_evidence':action.get('relationship_evidence'),'key':'|'.join((str(event.get('event_id')),name,f'{st:.6f}',target_semantic or target_event))}
...
def _step(phase,event,preset_name,role,**extra):return {'phase':phase,'event_id':event['event_id'],'preset':preset_name,'duration_seconds':duration(preset_name),'semantic_role':role,**extra}
...
def _pair_steps(subject,target,subject_state,target_state,action):
    if subject_state=='MIDDLE' and target_state=='RIGHT':return [_step('ACTION',subject,'WITHIN_MIDDLE_TO_LEFT','SOURCE_YIELDS_SPACE'),_step('REACTION',target,'WITHIN_RIGHT_TO_MIDDLE','TARGET_RECEIVES_FOCUS')],'CENTER_RIGHT_HANDOFF'
    if subject_state=='MIDDLE' and target_state=='LEFT':return [_step('ACTION',subject,'WITHIN_MIDDLE_TO_RIGHT','SOURCE_YIELDS_SPACE'),_step('REACTION',target,'WITHIN_LEFT_TO_MIDDLE','TARGET_RECEIVES_FOCUS')],'CENTER_LEFT_HANDOFF'
    if subject_state=='LEFT' and target_state=='MIDDLE':return [_step('ACTION',subject,'WITHIN_LEFT_TO_MIDDLE','SOURCE_APPROACHES_FOCUS'),_step('REACTION',target,'WITHIN_MIDDLE_TO_RIGHT','TARGET_YIELDS_AND_REACTS')],'LEFT_CENTER_EXCHANGE'
    if subject_state=='RIGHT' and target_state=='MIDDLE':return [_step('ACTION',subject,'WITHIN_RIGHT_TO_MIDDLE','SOURCE_APPROACHES_FOCUS'),_step('REACTION',target,'WITHIN_MIDDLE_TO_LEFT','TARGET_YIELDS_AND_REACTS')],'RIGHT_CENTER_EXCHANGE'
    if subject_state=='MIDDLE' and target_state=='MIDDLE':
        if action in {'COMPARE','BLOCK','REJECT'}:return [_step('ACTION',subject,'WITHIN_MIDDLE_TO_LEFT','SOURCE_ESTABLISHES_CONTRAST'),_step('REACTION',target,'WITHIN_MIDDLE_TO_RIGHT','TARGET_ESTABLISHES_COUNTERSTATE')],'CENTER_PAIR_DIVERGENCE'
...
def build_choreography_candidate(intent:dict,event_by_id:dict[str,dict],fps:float)->dict:
    ...
    if intent.get('physical_pair_allowed') and target:
        ...
        if fixed:return {'mode':'FIXED_EXISTING_PAIR','reason':None,'template':'SEMANTICALLY_ALIGNED_ENTRY_PAIR','adopted_actions':list(fixed),'steps':[]}
        ...
        if adopted:
            ...
            if reaction:
                return {'mode':'BASE_ACTION_PLUS_REACTION' if authored else 'ADOPTED_ENTRY_PLUS_REACTION','reason':None,'template':'AUTHORITY_BRIDGE_REACTION' if authored else 'SHORT_CARD_AUTHORITY_BRIDGE','adopted_action':adopted,
