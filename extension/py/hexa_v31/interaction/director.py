...
def _relationship_visual_guard(intent:dict,event_by_id:dict[str,dict],fps:float)->list[dict]:
    ...
    os=max(float(subject.get('physical_start_seconds',subject.get('start_seconds',0))),float(target.get('physical_start_seconds',target.get('start_seconds',0))));oe=min(float(subject.get('physical_end_seconds',subject.get('end_seconds',0))),float(target.get('physical_end_seconds',target.get('end_seconds',0))))
    ...
    for e in event_by_id.values():
        ...
        e['physical_start_seconds']=round(ns,6);e['physical_end_seconds']=round(ne,6);e['visibility_interval_seconds']=[round(ns,6),round(ne,6)];e['interaction_relationship_dependency']={'interaction_id':intent['interaction_id'],'source_event_id':cause_id or subject['event_id'],'target_event_id':reaction_id or target['event_id']};e['interaction_orphan_guard']='SOURCE_AND_TARGET_OVERLAP'
...
def _commit_actions(plan:dict,intent:dict,schedule:dict)->tuple[list[dict],list[dict]]:
    event_by_id={str(e.get('event_id')):e for e in plan.get('events') or []};committed=[];rejected=[];cause_id,reaction_id=_causal_ids(intent);reaction_event=event_by_id.get(str(reaction_id or intent.get('object_event_id') or ''));reaction_semantic_unit_id=(reaction_event or {}).get('semantic_unit_id');card_id=str(intent.get('visual_card_id'));card=next((c for c in (plan.get('visual_cards') or {}).get('cards',[]) if str(c.get('card_id'))==card_id),None);local=[e for e in event_by_id.values() if str(e.get('visual_card_id'))==card_id and not e.get('suppressed_by_card_density')];snapshots={str(e.get('event_id')):copy.deepcopy(e) for e in local}
    for step in schedule.get('steps') or []:
        ...
        e.setdefault('preset_actions',[]).append(action);e['preset_actions'].sort(key=lambda x:(float(x.get('start_seconds',0)),str(x.get('name'))));action_end=float(step['end_seconds']);e['motion_end_seconds']=round(max(float(e.get('motion_end_seconds',0)),action_end),6);e.setdefault('motion_intervals',[]).append({'kind':'ACTION',**action});committed.append({**step,'interaction_id':intent['interaction_id'],'semantic_action':intent['semantic_action'],'source_event_id':cause_id or intent['subject_event_id'],'target_event_id':reaction_id or intent.get('object_event_id'),'semantic_subject_event_id':intent.get('subject_event_id'),'semantic_object_event_id':intent.get('object_event_id'),'causal_direction':intent.get('causal_direction'),'swept_geometry':geo})
    if card:
...
def _adopt_existing_manifestation(plan:dict,intent:dict,row:dict)->tuple[dict|None,dict|None]:
    ...
    card_id=str(intent.get('visual_card_id') or event.get('visual_card_id') or '');local=[e for e in events.values() if str(e.get('visual_card_id'))==card_id and not e.get('suppressed_by_card_density')];geo=swept_path_report(event,str(row.get('preset')),float(row.get('start_seconds',0)),float(row.get('end_seconds',0)),local)
...
def apply_interaction_director(base_plan:dict,source_plan:dict,alignment:dict,fps:float=30.0,logger=None)->dict:
    plan=base_plan;compiled=compile_interaction_intents(plan,source_plan);intents=compiled['intents'];graph=build_interaction_graph(intents);event_by_id={str(e.get('event_id')):e for e in plan.get('events') or [] if not e.get('suppressed_by_card_density')};physical=[];schedules=[];fallbacks=[];orphan_guards=[];adopted_count=0;adopted_base_causes=0
    for intent in intents:
        ...
        if adopted_rows:
            ...
            for row in adopted_rows:
                ...
                adopted.append(live)
            schedule={'status':'ADOPTED_EXISTING_MOTION' if not failed else 'SAFE_FALLBACK_ADOPTION_FAILED','solver':'FIXED_EXISTING_MOTION','steps':[],'interaction_id':intent['interaction_id'],'candidate_mode':candidate.get('mode'),'candidate_reason':candidate.get('reason'),'candidate_template':candidate.get('template'),'causal_source_event_id':intent.get('causal_source_event_id'),'causal_target_event_id':intent.get('causal_target_event_id'),'causal_direction':intent.get('causal_direction')};schedules.append(schedule)
            if failed:fallbacks.append(failed)
            else:physical.extend(adopted);adopted_count+=len(adopted);adopted_base_causes+=sum(bool(x.get('adopted_from_base_plan')) for x in adopted)
            continue
        ...
        if schedule.get('status')=='COMMITTED':
            ...
            if committed:
                ...
                if adopt_rejection:fallbacks.append(adopt_rejection)
                    ...
                    if adopted:physical.append(adopted);adopted_count+=1;adopted_base_causes+=int(bool(adopted.get('adopted_from_base_plan')))
...
def build_interaction_motion_plan(plan:dict,alignment:dict,vision_results:list[dict],rules_path,reference_path,*,fps:float=30.0,logger=None,calibration:dict|None=None):
    ...
    base=base_build_motion_plan(plan,alignment,vision_results,rules_path,reference_path,fps=fps,logger=logger,calibration=calibration);return apply_interaction_director(base,plan,alignment,fps=fps,logger=logger)
