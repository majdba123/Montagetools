...
def _commit_actions(plan:dict,intent:dict,schedule:dict)->tuple[list[dict],list[dict]]:
    event_by_id={str(e.get('event_id')):e for e in plan.get('events') or []}
    committed=[];rejected=[]
    card_id=str(intent.get('visual_card_id'))
    card=next((c for c in (plan.get('visual_cards') or {}).get('cards',[]) if str(c.get('card_id'))==card_id),None)
    ...
    for step in schedule.get('steps') or []:
        ...
        action={'name':step['preset'],'start_seconds':float(step['start_seconds']),'duration_seconds':float(step['duration_seconds']),
                'authority':'HEXA_INTERACTION_DIRECTOR_V2','action_type':'INTERACTION_DIRECTOR','interaction_id':intent['interaction_id'],
                'interaction_phase':step['phase'],'semantic_action':intent['semantic_action'],
                'source_event_id':intent['subject_event_id'],'target_event_id':intent.get('object_event_id'),
                'relationship_evidence':intent.get('evidence'),'relationship_confidence':intent.get('confidence')}
        ...
        row={**step,'interaction_id':intent['interaction_id'],'semantic_action':intent['semantic_action'],
...
def apply_interaction_director(base_plan:dict,source_plan:dict,alignment:dict,fps:float=30.0,logger=None)->dict:
    plan=base_plan
    compiled=compile_interaction_intents(plan);intents=compiled['intents'];graph=build_interaction_graph(intents)
    event_by_id={str(e.get('event_id')):e for e in plan.get('events') or [] if not e.get('suppressed_by_card_density')}
    physical=[];schedules=[];fallbacks=[];orphan_guards=[]
    for intent in intents:
        ...
        schedule['interaction_id']=intent['interaction_id'];schedule['candidate_mode']=candidate.get('mode');schedule['candidate_reason']=candidate.get('reason')
        ...
        elif schedule.get('status')!='NO_PHYSICAL_STEPS':
            fallbacks.append({'interaction_id':intent['interaction_id'],'reason':schedule.get('status'),'detail':schedule.get('reason')})
    engine={'schema':'HEXA_INTERACTION_ENGINE_V2','version':INTERACTION_ENGINE_VERSION,'intents':intents,'graph':graph,
            'schedules':schedules,'physical_actions':physical,'safe_fallbacks':fallbacks,'relationship_orphan_guards':orphan_guards,
            'logical_interaction_count':len(intents),'physical_interaction_count':len(set(x['interaction_id'] for x in physical)),
            'physical_action_count':len(physical),'ortools_required':True,'shapely_required':True,
            'deterministic_solver_contract':{'num_search_workers':1,'random_seed':0,'bounded_seconds_per_interaction':.20}}
    plan['interaction_engine']=engine
    qa=interaction_plan_qa(plan);plan['interaction_plan_qa']=qa
    ...
    plan['motion_dna_version']=str(plan.get('motion_dna_version') or 'HEXA_MOTION_DNA_V31')+'__INTERACTION_V2'
    plan.setdefault('hard_invariants',{})['interaction_execution_authority_required']=True
    plan['hard_invariants']['interaction_encoded_pixel_verification_required']=True
    plan.setdefault('budget_summary',{})['interaction_logical_count']=engine['logical_interaction_count']
    plan['budget_summary']['interaction_physical_action_count']=engine['physical_action_count']
    if not qa.get('pass'):
        raise ValueError('INTERACTION_PLAN_QA_FAILED: '+str(qa.get('failures')[:6]))
