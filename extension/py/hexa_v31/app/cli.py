...
from hexa_v31.pipeline import build, BuildFailure
from hexa_v31.package_io import open_and_validate
from hexa_v31.util import ensure_dir, read_json
from hexa_v31.production_cert import certify_production
from hexa_v31.app.motion_preflight import motion_preflight
...
def main(argv=None):
    ...
    m=sub.add_parser('motion-preflight'); m.add_argument('--package',required=True); m.add_argument('--voice',required=True); m.add_argument('--work-root',required=True); m.add_argument('--extension-root',required=True)
    ...
    try:
        ...
        if args.cmd=='motion-preflight':
            r=motion_preflight(args.package,args.voice,args.work_root,args.extension_root);print(r['status']);print('HEXA_V31_MOTION_PREFLIGHT_JSON='+json.dumps(r,ensure_ascii=False));return 0 if r['status']=='MOTION_PREFLIGHT_PASS' else 4
