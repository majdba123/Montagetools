...
from . import VERSION, ENGINE_ID
from .util import ensure_dir, read_json, sha256_file, write_json, documents_dir, safe_filename
...
from .typography import build_text_plan, find_arabic_font, merge_support_typography
from .scene_media import render_scene_media, assemble_final_mp4
from .reference_metrics import analyze_video, score_against_reference_floor
from .graphics import build_graphics_plan
from .production_cert import certify_production
from .orchestration import balance_presentation
from .qa import build_qa_report, motion_rule_qa, alignment_qa, reference_plan_qa
from .preset_qa import preset_motion_qa, preset_story_plan_qa
from .storytelling_verify import verify_storytelling_render
from .spike_attribution import attribute_spikes
from .perceptual_qa import evaluate_perceptual_story_from_metrics
from .physical_acting_verify import verify_physical_acting
from .reference_critic import score_reference_10
...
def build(scene_package_zip:str, voice_over:str, work_root:str|None=None, extension_root:str|None=None, echo=True):
    ext=pathlib.Path(extension_root or pathlib.Path(__file__).resolve().parents[2]).resolve()
    ...
    try:
        ...
        for i,s in enumerate(pkg.scenes,1):
            ...
            cmd=[sys.executable,'-m','hexa_v31.vision_worker','--scene-json',str(spec),'--image',str(img),'--out-dir',str(vroot)]
