...
for p in [ROOT/'tools/install_v31.py',ROOT/'tools/selftest_v31.py']:ast.parse(p.read_text(encoding='utf-8'));count+=1
ET.parse(ROOT/'extension/CSXS/manifest.xml');print('PYTHON_XML_SYNTAX_PASS',count)
run(['node','--check',ROOT/'extension/js/main.js'])
host=(ROOT/'extension/jsx/host.jsx').read_text(encoding='utf-8');cp=subprocess.run(['node','--check'],input=host,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT);print(cp.stdout);assert cp.returncode==0;print('HOST_JSX_SYNTAX_PASS')
