...
def centroid(frame, rgb):
    # Codec-tolerant color identity tracking; H.264 may shift exact RGB values.
    if rgb[0] > rgb[2]:
        mask=(frame[:,:,2] > 115) & (frame[:,:,2] > frame[:,:,0]*1.35)
    else:
        mask=(frame[:,:,0] > 115) & (frame[:,:,0] > frame[:,:,2]*1.35)
    yy,xx=np.where(mask)
    assert len(xx)>30
...
with tempfile.TemporaryDirectory() as raw:
    ...
    manifest=render_scene_media({'events':[event('red',paths[0],'ENTRY_LEFT_TO_MIDDLE',[{'name':'WITHIN_MIDDLE_TO_LEFT','start_seconds':1.5,'duration_seconds':0.8}]),event('blue',paths[1],'ENTRY_RIGHT_TO_MIDDLE',[{'name':'WITHIN_MIDDLE_TO_RIGHT','start_seconds':1.5,'duration_seconds':0.8}])]},motion,[],{'events':[]},{'events':[]},td/'out',td/'cache',width=640,height=360,fps=30)
    cap=cv2.VideoCapture(manifest['clips'][0]['source_path']);frames=[]
    # Both probes are inside the entry but after the initially off-canvas state.
    for n in (28,70): cap.set(cv2.CAP_PROP_POS_FRAMES,n);ok,frame=cap.read();assert ok;frames.append(frame)
