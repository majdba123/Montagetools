...
media_facade = (root/'extension/py/hexa_v31/scene_media.py').read_text(encoding='utf-8')
...
# Pixel cache must invalidate when rendering/typography evaluator code changes even
# if the semantic text/motion plan itself is byte-identical.
assert '_renderer_dependency_signature' in media_facade
assert 'scene_media_pixels_' in media_facade
assert 'render_text_rgba' in media_facade
assert 'renderer_dependency_sha256' in media_facade
