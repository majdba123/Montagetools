...
from hexa_v31.layout.reference_perceptual_residual import (
    finalize_reference_perceptual_residual,
)
...
def event(eid, x, y, bbox, *, primary=False, start=0.0, end=4.0, scene='SCENE_GENERIC'):
    row = {
        ...
        'settle_seconds': min(end, start + 0.4),
        'perceptual_hit_seconds': start + 0.7,
        'preset_entry': None,
        'preset_exit': None,
        'preset_actions': [],
...
def plan(events, *, end=4.0):
    return {
        'fps': 30.0,
        'events': events,
        'visual_cards': {'cards': [card(events, end=end)]},
    }


# A single small composite/focal source is the production class that pair/cohort
# fitters cannot solve. Closure must reach a perceptually material card target,
# not merely increment the previous scale by a few percent.
single = event('SINGLE_ROOT', .50, .52, (0, 0, .13, .18), primary=True)
single_plan = plan([single])
single_stats = finalize_reference_perceptual_residual(single_plan)
...
# A moderately sparse two-root semantic cohort should be enlarged as one card
# composition while preserving hierarchy and collision safety.
primary = event('GROUP_PRIMARY', .30, .52, (0, 0, .22, .26), primary=True)
support = event('GROUP_SUPPORT', .70, .52, (0, 0, .22, .26))
group_plan = plan([primary, support])
group_stats = finalize_reference_perceptual_residual(group_plan)
...
assert group_stats['after_mean_projected_ink'] >= .22, group_stats
...
assert support['layout_scale_multiplier'] > 1.0
...
# True center travel is a protected P2 authority. Use a baseline that is itself
# valid so this verifies no-op protection rather than tolerating a bad fixture.
travel_owner = event('TRAVEL_OWNER', .20, .52, (0, 0, .14, .22), primary=True)
travel_target = event('TRAVEL_TARGET', .50, .52, (0, 0, .10, .14))
...
# Partition ownership remains atomic and completely outside this final root pass.
child = event('PARTITION_CHILD', .42, .52, (0, 0, .10, .15), primary=True)
child['render_mode'] = 'CHILD_PARTITION'
child['partition_group_id'] = 'PG'
residual = event('PARTITION_RESIDUAL', .62, .52, (0, 0, .10, .15))
residual['render_mode'] = 'RESIDUAL_SUPPORT'
residual['partition_group_id'] = 'PG'
partition_plan = plan([child, residual])
partition_before = copy.deepcopy(partition_plan)
partition_stats = finalize_reference_perceptual_residual(partition_plan)
...
# Geometry selection is deterministic and independent of package/event names.
other = event('COMPLETELY_DIFFERENT_ID', .50, .52, (0, 0, .13, .18), primary=True)
other_plan = plan([other])
other_stats = finalize_reference_perceptual_residual(other_plan)
