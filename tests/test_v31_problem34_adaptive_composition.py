...
with tempfile.TemporaryDirectory(prefix='hexa_problem34_') as raw:
    ...
    participant_a,participant_b=b['composition_participant_states']
    assert participant_a['start_seconds']+participant_a['transition_duration_seconds']<=participant_b['start_seconds']+1e-6,b
    assert composition_state_at(b,3.5)[1]==1.10,b
    assert abs(_event_state(b,3.5)[1]-1.10)<1e-6,b
    intervals,_,_=_compile_final_motion_intervals(b)
    assert sum(row['kind']=='COMPOSITION_PARTICIPANT_STATE' for row in intervals)==2,intervals
    assert build_visual_density_report({'events':[a,b],'visual_cards':cards})['cards'][0]['peak_visible_object_count']>=2
    sealed=_final_timing_sha256({'events':[a,b]});changed=copy.deepcopy(b)
    changed['composition_participant_states'][1]['start_seconds']+=.1
    assert _final_timing_sha256({'events':[a,changed]})!=sealed
    ...
    serialized=event('SERIALIZED',.50,.5,a['source_path'],False);serialized['attention_priority']='PRIMARY';serialized['end_seconds']=2.0;serialized['physical_end_seconds']=5.0;serialized['motion_end_seconds']=2.0;serialized['preset_exit']={'name':'DISAPPEAR_DOWN_SCALE','start_seconds':1.4,'duration_seconds':.6};late=event('LATE',.72,2.8,b['source_path'],False);late['start_seconds']=2.1;late['physical_start_seconds']=2.1;late['motion_start_seconds']=2.1;late['settle_seconds']=2.9
    serialized_stats=_adaptive_composition_state_optimize([serialized,late],cards,30.0);assert serialized_stats['candidates_committed']==0 and serialized_stats['rejections'].get('DENSITY_CONCURRENCY')==1,serialized_stats;assert not serialized.get('composition_states') and not late.get('composition_participant_states'),(serialized,late)
    ...
    edit={'events':[copy.deepcopy(a),copy.deepcopy(b)]};out=root/'out';cache=root/'cache';manifest=render_scene_media(edit,motion,[],{'events':[]},{'events':[]},out,cache,width=640,height=360,fps=30.0);video=manifest['clips'][0]['source_path'];qa=verify_encoded_composition(video,motion,{'median_estimated_alpha_coverage':.03},30.0,render_edit_map=edit)
