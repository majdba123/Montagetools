...
def event(eid,x,role='PRIMARY',semantic_intent=''):
    return {'event_id':eid,'scene_id':'ARBITRARY_SCENE','visual_card_id':'ARBITRARY_CARD','semantic_unit_id':eid,'semantic_scope_id':'ARBITRARY_SCENE::'+eid,'semantic_type':'CONCEPT','semantic_role':role,'attention_priority':role,'semantic_intent':semantic_intent,'semantic_mapping_confidence':.99,'translation_safe_after_occlusion':True,'animation_safe':True,'card_rest_position_norm':[x,.493],'planned_rect_norm':[x-.055,.433,.11,.12],'source_bbox_norm':[x-.055,.433,.11,.12],'layout_scale_multiplier':1.0,'reference_camera_scale':1.0,'start_seconds':0.0,'settle_seconds':.35,'end_seconds':4.4,'physical_start_seconds':0.0,'physical_end_seconds':4.4,'motion_end_seconds':.35,'preset_entry':None,'preset_exit':None,'preset_actions':[],'motion_intervals':[],'perceptual_hit_seconds':1.0,'render_mode':'ROOT_ATOMIC'}
...
source_plan={'scenes':[{'scene_id':'ARBITRARY_SCENE','units':[{'unit_id':'SOURCE'},{'unit_id':'TARGET'}],'visual_progression':[{'targets':['SOURCE','TARGET']}]}]}
...
assert a['interaction_plan_qa']['pass'],a['interaction_plan_qa'];engine=a['interaction_engine'];assert engine['logical_interaction_count']==1,engine;assert engine['physical_action_count']==2,engine;assert engine['intent_compiler']['physical_pair_candidate_count']==1
phases=[x['phase'] for x in engine['physical_actions']];assert phases==['ACTION','REACTION'],phases
action,reaction=engine['physical_actions'];assert reaction['start_seconds']>=action['end_seconds']+1/30-1e-6,(action,reaction);assert all((x.get('swept_geometry') or {}).get('pass') for x in engine['physical_actions'])
